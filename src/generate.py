...
def get_hh(split: str, sanity_check: bool = False, silent: bool = False, cache_dir: Optional[str] = None) -> Dataset:
    ...
    """
    if split == 'train':
        dataset = load_dataset('json', data_files={'train': "generated/HH_PKU_10k_demonstration.json"})
    ...
    def split_prompt_and_responses_new(sample) -> Dict[str, str]:
        # prompt = extract_anthropic_hh_prompt(sample["chosen"])
        
        return {
        "real": [{"role": "user", "content": sample['prompts']}, 
                {"role": "assistant", "content": sample["chosen"]}], 
        "generated": [{"role": "user", "content": sample['prompts']}, 
    ...
    if split == 'test':
        ...
        return dataset.map(split_prompt_and_responses_new)
...
def main():
    ...
    elif args.input_dir == 'Anthropic/hh-rlhf' or args.input_dir == 'generated/HH_PKU_10k_demonstration.json':
