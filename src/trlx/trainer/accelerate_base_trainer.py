...
class AccelerateRLTrainer(BaseRLTrainer):
    ...
    def __init__(self, config, **kwargs):  # noqa: C901
        ...
        self.model = self.setup_model(config.policy_model_path)
        if hasattr(config, 'ref_model_path'):
            self.ref_model = self.setup_model(config.ref_model_path)
        else:
            self.ref_model = None
        # self.ref_model = self.setup_model(config.ref_model_path)
    ...
    def setup_model(self,policy_model_path=None):
        """
        ...
        if policy_model_path != None:
            x = model.from_pretrained(policy_model_path)
            model = x
    ...
    def generate_response_no_eval(self):  # noqa: C901
        ...
        for _ in range(self.config.train.epochs):
            ...
            for batch in self.train_dataloader:
                ...
                for _ in range(self.n_updates_per_batch):
                    ...
                    if self.iter_count % self.config.train.eval_interval == 0 and not self.iter_count==0:
                        results = self.evaluate()
