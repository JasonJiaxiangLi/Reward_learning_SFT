...
from alignment import SPINTrainer, NewSPINTrainer, SFTTrainer
...
def main():
    ...
    print(f"raw_datasets structure: {raw_datasets}")
    # exit()
    ...
    if model_args.use_peft is True:
        ...
        peft_config.target_modules = ["q_proj","k_proj","v_proj","o_proj","gate_proj","up_proj","down_proj","lm_head"]
        print(peft_config)
    else:
        peft_config = None
    ...
    #########################
    if training_args.set_trainer == 'new_spin':
        spin_trainer = NewSPINTrainer(
            model,
            ref_model,
        ...
        print(dir(spin_trainer))
        # exit()
    elif training_args.set_trainer == 'sft':
        spin_trainer = SFTTrainer(
            model,
            ref_model,
            model_init_kwargs=model_kwargs,
            ref_model_init_kwargs=ref_model_kwargs,
    ...
    # test_results = spin_trainer.evaluate()
    # print(f"test results: {test_results}")
    # spin_trainer.log_metrics("test", test_results)
