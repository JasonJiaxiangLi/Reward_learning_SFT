...
class SPINConfig(transformers.TrainingArguments):
    ...
    alpha: Optional[float] = field(
        default=0.0,
        metadata={"help": "The alpha factor in SPIN loss which regularizes the loss"},
    )
    generation_rate: Optional[float] = field(
        default=1.0,
        metadata={"help": "rate of generating while training"},
    )
    generate_interval: Optional[int] = field(
        default=1,
        metadata={"help": "rate of generating while training"},
    )
    set_trainer: Optional[bool] = field(
        default="spin",
        metadata={"help": "the trainer you want to use, ['sft', 'spin', 'new_spin']"},
    )
    ...
    max_new_tokens: Optional[int] = field(
        default=128,
        metadata={"help": "maximum generation length for training"},
    )
    optim: Optional[str] = field(default="rmsprop")
    remove_unused_columns: bool = field(default=False)
    save_steps: Optional[int] = field(
        default=0,
        metadata={"help": "save for every this number of steps"},
    )
    # stages: Optional[int] = field(
    #     default=1,
    #     metadata={"help": "the total number of stages during the training"},
    # )